...
export async function enterMarkets(
  ...
  await netId(this);
  const errorPrefix = 'Fortress [enterMarkets] | ';
  ...
  for (let i = 0; i < markets.length; i++) {
    if (markets[i][0] !== 'f') {
      markets[i] = 'f' + markets[i];
    }

    if (!cTokens.includes(markets[i])) {
      throw Error(errorPrefix + 'Provided market `' + markets[i] + '` is not a recognized fToken.');
    }
...
export async function exitMarket(
  ...
  await netId(this);
  const errorPrefix = 'Fortress [exitMarkets] | ';
