...
/**
 * Submit a vote on a Fortress Governance proposal.
 *
 ...
 * 
 * (async function() {
 *   const castVoteTx = await fortress.castVote(12, true);
...
export async function castVote(
  ...
  const errorPrefix = 'Fortress [castVote] | ';
...
/**
 * Submit a vote on a Fortress Governance proposal using an EIP-712 signature.
 *
 * @param {string} proposalId The ID of the proposal to vote on. This is an
 *     auto-incrementing integer in the Governor Alpha contract.
 * @param {boolean} support A boolean of true for 'yes' or false for 'no' on the
 ...
 * 
 * (async function() {
 *   const castVoteTx = await fortress.castVoteBySig(
 *     12,
 *     true,
 *     {
 *       v: '0x1b',
...
export async function castVoteBySig(
  ...
  const errorPrefix = 'Fortress [castVoteBySig] | ';
...
/**
 * Create a vote signature for a Fortress Governance proposal using EIP-712.
 ...
 * (async () => {
 *
 *   const voteForSignature = await fortress.createVoteSignature(20, true);
 *   console.log('voteForSignature', voteForSignature);
 *
 *   const voteAgainstSignature = await fortress.createVoteSignature(20, false);
